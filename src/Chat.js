...
function Chat() {
    ...
    useEffect(() => {
      if (roomId) {
          ...
          db.collection("rooms").doc(roomId)
          .collection('messages').orderBy("timestamp", "asc").onSnapshot((snapshot) => setMessages(snapshot.docs.map((doc) => doc.data()))
          );
       }
    }, [roomId]);
            ...
            <MoreVertIcon />
              ...
              <div className="chat__body">
                  {messages.map((message) => (
                      ...
                      {message.message}
                     <span 
                     className="chat__timestamp">{new Date(message.timestamp?.toDate()).toUTCString()}
                     </span>
